    ...
    <>
      ...
      <ListGroup>
        {items.map((item, index) => (
          <ListGroup.Item
            key={item.order_summary_id}
            className="d-flex justify-content-between align-items-center"
          >
            <div className="d-flex align-items-center">
              <input
                type="checkbox"
                checked={selectedItems.includes(item.order_summary_id)}
                onChange={() => handleSelectItem(item.order_summary_id)}
                id={`checkbox-${item.order_summary_id}`} // 고유한 ID 추가
                style={{ marginRight: "0.5rem" }} // 체크박스와 라벨 간격
              />
              <label
                htmlFor={`checkbox-${item.order_summary_id}`} // 클릭 시 체크박스 선택
                style={{ cursor: "pointer", marginBottom: 0 }} // 포인터 커서 추가
              >
                총합: {item.total_amount}
              </label>
            ...
            <Button
              variant="dark"
              className="ml-2"
              onClick={() => handleDelete(item.order_summary_id)}
            >
              삭제
            </Button>
          </ListGroup.Item>
