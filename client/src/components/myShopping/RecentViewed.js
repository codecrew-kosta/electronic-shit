...
import ItemList from "./ItemList_recent";
...
const RecentViewed = () => {
  ...
  const fetchItems = async () => {
    try {
      const response = await fetch("http://localhost:3001/product/recentview?user_id=1");
      if (!response.ok) {
      ...
      console.log("받은 데이터:", result);  // 응답 본문을 확인
  ...
  return (
    <Container className="mt-5">
      <h2 className="text-center mb-4">최근 본 상품</h2>
      <Row>
        <Col>
          <ItemList
            items={items}
